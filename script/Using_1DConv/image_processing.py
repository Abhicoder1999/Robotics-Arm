...
while True:
    ...
    contour =     
    polar = cv2.logPolar(edge, (edge.shape[0]/2, edge.shape[1]/2), 40, cv2.WARP_FILL_OUTLIERS)  
    #dist = cv2.distanceTransform(closing, cv2.DIST_L2, 3)
    #laplacian = cv2.Laplacian(np.uint8(dist),cv2.CV_64F)
    #check = abs(laplacian) > np.amax(laplacian)-1
    # check = check*1
    cv2.imshow("edges",polar)
